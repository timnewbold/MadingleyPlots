...
PlotGrid <- function(resultsDir,plotName,outDir,
                     label=NULL,
                     vars=c("Biomass density"),endTimeStep=NULL,
                     numTimeSteps=12,
  ...
  for (i in varsToTransformPlantBiomass){
    resultsSimMean[i,,]<-resultsSimMean[i,,]/
      .GetLPRatios(resultsDir)
  }
  
  # Sum values across the specified variables
  resultsTotal<-apply(resultsSimMean,c(2,3),sum,na.rm=TRUE)
  
  resultsTotal[resultsTotal==0]<-NA
  
  resultsMap<-SpatialGridDataFrame(grid = GridTopology(
    cellcentre.offset = c(min(long),min(lat)),
    cellsize = c(diff(long)[1],diff(lat)[1]),
    cells.dim = c(length(long),length(lat))),
    data = data.frame(band1=as.vector(resultsTotal[,ncol(resultsTotal):1])))
  
  .Log("Plotting\n")
  
  brks <- quantile(resultsTotal,probs = seq(from=0,to=1,length.out=11),
                   na.rm = TRUE)
  ...
  resultsTimesMean[resultsTimesMean==0]<-NA
  
  if (returnMap){
    return(list(mean=resultsMap,simulations=resultsTimesMean))
  }
